...
public class CarModelOptionsIO {
		...
		return auto;
			...
			public Automobile readProperty(Properties prop){
				...
				auto = new Automobile(prop.getProperty("CarMake"),
						prop.getProperty("CarModel"), Float.parseFloat(prop
				...
				for (char optNum = '1'; prop.getProperty(option + optNum) != null; optNum++) {
					...
					for (char optValueNum = 'a'; prop.getProperty(optionValue
							+ optNum + optValueNum) != null; optValueNum++) {
						auto.setOption(
								prop.getProperty(option + optNum),
								prop.getProperty(optionValue + optNum
										+ optValueNum),
								Float.parseFloat(prop.getProperty(optionPrice
										+ optNum + optValueNum)));
					}
				}
				return auto;
			...
			public Automobile readAuto(String filename) throws Exception{
	...
	public String[] getAutoFileList(String fileName){
		...
		String[] autoNameList = new String[autoList.size()];
		for (int i = 0; i < autoNameList.length; i++) {
			autoNameList[i] = autoList.get(i);
		}
		return autoNameList;
	}
