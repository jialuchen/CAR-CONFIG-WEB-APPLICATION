...
class OptionSet implements Serializable {
	...
	protected Option findOption(int index){
	...
	protected LinkedHashMap<String, Float> getAllOption() {
		LinkedHashMap<String, Float> optionsetmap = new LinkedHashMap<String,Float>();
		for(Option op : opt){
			//System.out.println(op.getOptionName());
			//System.out.println(op.getOptionPrice());
			optionsetmap.put(op.getOptionName(), op.getOptionPrice());
	...
	protected void printOptions() {
		for (int i =0; i< opt.size(); i++) {
			System.out.printf("%d. Option is %s, its price is %.1f\n", i, opt.get(i).getOptionName(), opt.get(i).getOptionPrice());
		}
	}
